;;======= Δήλωση Μεταβλητών ========;;
globals [
  neighbor-radius ;; Η εμβέλεια που μπορούν να αντιληφθούν οι άνθρωποι άλλους ανθρώπους
]
...
turtles-own [
  ...
  reset-level
  window
  is-machine? ;; true για τα μηχανήματα
]
...
to setup
  ...
  ;; Δημιουργία κοινού (πυγολαμπίδες/ανθρώπων)
  ifelse theatre-grid? [
    place-people-grid
  ]
  [
    create-turtles number [
      ...
      set is-machine? false
      set size 2
      recolor
    ]

    set neighbor-radius 1
  ]
...
to place-people-grid
  ;; Υπολογισμός διαστάσεων θεάτρου
  let rows floor (sqrt number)
  let cols ceiling (number / rows)
  let spacing-x (max-pxcor * 2) / cols
  let spacing-y (max-pycor * 2) / rows
  let start-x min-pxcor + spacing-x / 2
  let start-y max-pycor - spacing-y / 2
  set neighbor-radius min (list spacing-x spacing-y) * 1.5 ;; Θεωρούμε ότι οι άνθρωποι μπορούν να ακούσουν
                                                           ;; περίπου 2 θέσεις γύρω τους άλλους ανθρώπους

  ;; Δημιουργία πυγολαμπίδων (θεατών) σε grid
  let cnt 0
  repeat number [
    let i cnt mod cols
    let j floor (cnt / cols)
    create-turtles 1 [
      setxy (start-x + i * spacing-x) (start-y - j * spacing-y)
      set shape "person"
      set size 2
      set is-machine? false
      set clock random (round cycle-length)
...
;; Δημιουργεί μηχανές ήχου στην θέση του ποντικιού.
to place-machine
  if mouse-down? [
    create-turtles 1 [
      setxy mouse-xcor mouse-ycor
      set shape "target"
      set clock 0  ;; Οι μηχανές ξεκινάνε όλες μαζι να παίζουν τον ήχο
      set threshold flash-length
      set reset-level 0  ;; Δεν αλλάζει το ρολόι τους
      set window -1
      set is-machine? true
      set size 3
      recolor
...
;; Διαγράφει μηχανές ήχου στην θέση του ποντικιού.
to delete-machine
  if mouse-down? [
    let target one-of turtles with [
      is-machine? and distancexy mouse-xcor mouse-ycor < 1
    ]
    if target != nobody [
      ask target [ die ]
    ]
...
to look
  if not is-machine? [
    let machine-is-flashing count turtles in-radius machine-radius with [ color = red ]
    ifelse machine-is-flashing >= 1 [
      set clock 0
    ]
    [
      let flashing-neighbors count other turtles in-radius neighbor-radius with [ color = yellow ]
      if flashing-neighbors >= flashes-to-reset [
...
PENS
"flashing" 1.0 0 -2674135 true "" "plot count turtles with [color = yellow and not is-machine?]"
...
1095
101
Κρατόντας πατημένο το αριστερό κλικ του ποντικιού πάνω σε ένα σημείο στο World View και πατόντας το Α ή το D προστήθεται μία μηχανή ήχου στο σημείο εκείνο ή αφαιρέιται αντίστοιχα.
10
0.0
1
...
1165
150
Ελέγχει αν οι άνθρωποι μπορεί να κουνιθούν και τους τοποθετοί σαν βρίσκονται σε θέσεις θεάτρου. 
10
0.0
1
...
1075
201
Ελέγεχει την εμβέλεια των μηχανών ήχου.
10
0.0
1
